	...
	callback = function()
		...
		if found:match("%.gdproject$") then
			...
			if f then
				local line = f:read("*l")
				f:close()
				local project_folder = line:match("=%s*(%S+)")
					or line:match("(%S+)")
		...
		if not server_started or server_address ~= addr then
			...
			if success then
				vim.notify(
					"Godot Server started @: " .. server_address,
					vim.log.levels.INFO
				)
				server_started = true
			end
...
autocmd("VimLeave", {
	callback = function()
		if server_started then vim.fn.serverstop(server_address) end
	end,
})
